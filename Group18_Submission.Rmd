...
## Deactivation of Scientific Notation
...
Outliers can have a large influence on statistics derived from the dataset which is why they need to be dealt with. The variable *tripduration* consists of the time (in seconds) a customer gets a bicycle from citi bike until the bike is returned to a station with an empty dock. Hence, there could be natural as well as non-natural outliers. While the non-natural outliers stem from measurement errors (system failures and similar), the cause of natural outliers could stem from customers not returning the rented bike, customers who could not return the rented bike or similar.
...
hist(citi_cb$tripduration)
hist(citi_cb$meantemp)
hist(citi_cb$PRCP)
...
citi_cb <- subset(citi_cb, tripduration < 6000)
...
* To display maps in a pdf document or any other format except HTML, static maps need to be used. Leaflet is a JavaScript library for building interactive maps for the web and hence not static.
